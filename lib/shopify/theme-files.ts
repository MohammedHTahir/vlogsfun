...
function settingsSchema(projectName: string, css: string): string {
  const brand = pickColor(css, 'brand', '#0D1117');
  const accent = pickColor(css, 'accent', '#0D1117');
  const schema = [
    {
      name: 'theme_info',
      theme_name: `${projectName} (vlogs.fun)`.slice(0, 60),
      theme_version: SHOPIFY_THEME_VERSION,
      theme_author: 'vlogs.fun',
...
export function buildThemeFiles(input: ThemeExportInput, converted?: ConvertedContent): ThemeBuild {
  ...
  pushFile(
    text(
      'assets/theme.css',
      `/* Project global theme — generated by vlogs.fun */\n${input.themeCss || ''}\n`
    )
  );
