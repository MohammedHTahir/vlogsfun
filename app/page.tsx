    ...
    <div className="relative min-h-screen overflow-hidden bg-[#fffdfc]">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[960px] flex-col px-8 py-8">
        ...
        <section className="mx-auto w-full max-w-[780px]">
          ...
          <div className="mb-7 rounded-2xl border border-[#e8e2de] bg-white p-4 shadow-[0_18px_40px_rgba(31,41,55,0.07)]">
            <textarea
              aria-label="Describe a Shopify page or theme"
              placeholder="Ask me to build a Shopify page or theme..."
              className="h-[82px] w-full resize-none border-0 bg-transparent px-2 py-2 text-base text-[#111827] outline-none placeholder:text-[#7b8492]"
            />
            <div className="flex items-end justify-between">
              <div className="flex gap-2">

                <button aria-label="Mention" className="grid h-10 w-11 place-items-center rounded-xl border border-[#e8e2de] bg-white text-[#4b5563] transition hover:bg-[#fff8f5]">
                  <Plus size={16} strokeWidth={1.8} />
                </button>
              </div>
              <button aria-label="Submit prompt" className="grid h-11 w-11 place-items-center rounded-xl bg-[#ff6747] text-white shadow-[0_12px_22px_rgba(255,103,71,0.2)] transition hover:bg-[#f85b3a]">
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2">
            {templateCards.map((card) => {
              ...
              return (
                <button
                  key={card.title}
                  className="group flex min-h-[100px] items-center justify-between gap-4 rounded-2xl border border-[#eee7e3] bg-white p-4 text-left shadow-[0_10px_24px_rgba(31,41,55,0.035)] transition hover:border-[#ffd4c7] hover:shadow-[0_16px_32px_rgba(31,41,55,0.06)]"
                >
